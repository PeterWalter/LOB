...
namespace CETAP_LOB.Model.scoring
{
  public class Dir_categories
  {
    ...
    private List<string> _ALScorefiles = new List<string>();
    private List<string> _QLScorefiles = new List<string>();
    ...
    public List<string> AQLScorefiles
    {
      ...
      set
      {
        ...
        public List<string> ALScorefiles
        {
            get
            {
                return _ALScorefiles;
            }
            set
            {
                _ALScorefiles = value;
        ...
        public List<string> QLScorefiles
        {
            get
            {
                return _QLScorefiles;
            }
            set
            {
                _QLScorefiles = value;
    ...
    private void SortFiles(List<string> thefiles)
    {
      foreach (string thefile in thefiles)
      {
        ...
        if (Path.GetExtension(thefile) == ".xlsx")
        {
          ...
          string the_name = str[0];
          string[] str2 = the_name.Split('_');

          switch (str[0].Substring(0, 3))
          {
            case "NBT":
              if (str[0].Substring(0, 7) == "NBT_Ans")
              {
              ...
              break;
            case "AQL":
                      if (str2[1] == "AL") ALScorefiles.Add(thefile);
                      if (str2[1] == "QL") QLScorefiles.Add(thefile);
