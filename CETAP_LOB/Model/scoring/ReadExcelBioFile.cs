      ...
      foreach (IXLTableRow row in (IEnumerable<IXLTableRow>) xlWorksheet.Range(xlWorksheet.FirstCellUsed().Address, xlWorksheet.LastCellUsed().Address).AsTable().DataRange.Rows((Func<IXLTableRow, bool>) null))
      {
        ...
        if (!row.Field("MAT_LANG").IsEmpty())
          myBio.Mat_Language = row.Field("MAT_LANG").GetString().Trim();
                ...
                //string provinceValue = row.Field("Province").GetString()?.Trim();

                //if (string.IsNullOrEmpty(provinceValue))
                //{
                //    myBio.ProvinceCode = null;
                //}
                //else
                //{
                //    myBio.ProvinceCode = new int?(Convert.ToInt32(provinceValue));
                //}

                if (!row.Field("Province").IsEmpty())
                    myBio.ProvinceCode = new int?(Convert.ToInt32(row.Field("Province").GetString().Trim()));
